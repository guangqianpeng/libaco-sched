...
//
// Modifications copyright 2019 guangqianpeng <guangqian1994@foxmail.com>
//
...
// aco's pointer passing between caller (resumer) and callee (yielder) 
__thread void* acosw_arg = NULL;
...
void aco_share_stack_destroy(aco_share_stack_t* sstk){
    ...
    assert(sstk->use_count == 0);
    free(sstk);
}

void aco_share_stack_unref(aco_share_stack_t* sstk) {
    sstk->use_count--;
    if (sstk->use_count == 0) {
...
aco_attr_no_asan
void* aco_resume(aco_t* resume_co, void* arg){
    assert(resume_co != NULL && resume_co->main_co != NULL
        && resume_co->is_end == 0
    );
    assert(resume_co->is_started || arg == NULL);
    ...
    resume_co->is_started = 1;
    resume_co->is_running = 1;
    resume_co->main_co->is_running = 0;
    ...
    resume_co->is_running = 0;
    resume_co->main_co->is_running = 1;

    return acosw_arg;
}

void* aco_yield1(aco_t* yield_co, void* arg) {
    aco_assertptr(yield_co);
    aco_assertptr(yield_co->main_co);

    yield_co->is_running = 0;
    yield_co->main_co->is_running = 1;
    acosw_arg = arg;

    acosw(yield_co, yield_co->main_co);

    // never get here if yield_co is end
    aco_assert(!yield_co->is_end);
    yield_co->is_running = 1;
    yield_co->main_co->is_running = 0;
...
void aco_destroy(aco_t* co){
    ...
    if(aco_is_main_co(co)){
        ...
        aco_share_stack_unref(co->share_stack);
